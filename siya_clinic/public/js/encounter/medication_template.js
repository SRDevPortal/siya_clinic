// siya_clinic/public/js/encounter/medication_template.js
...
console.log("Medication Template Loader loaded");
...
frappe.ui.form.on('Patient Encounter', {
  ...
  load_template_medication(frm) {
    if (!frm.doc.sr_medication_template) {
      clear_all_medication_tables(frm);
      return;
    }
    ...
    const resolve_child_table = (medication_class = "") => {
        const cls = (medication_class || "").toLowerCase().trim();

        if (["ayurvedic", "ayurvedic medicine"].includes(cls)) {
            return "drug_prescription";
        }
        if (["homeopathic", "homeopathic medicine"].includes(cls)) {
            return "sr_homeopathy_drug_prescription";
        }
        if (["allopathic", "allopathic medicine"].includes(cls)) {
      ...
      callback(r) {
        ...
        (tmpl.sr_medications || []).forEach(row => {
          ...
          if (!target_table || !frm.fields_dict[target_table]) {
            console.warn("Skipped medication due to invalid class:", row.sr_medication);
            return;
          }
          ...
          if (hasField("no_of_tablets_per_day_for_calculation")) med.no_of_tablets_per_day_for_calculation = per_day;
...
/* ---------------- CLEAR TABLES HELPER ---------------- */
function clear_all_medication_tables(frm) {
  [
    "drug_prescription",
